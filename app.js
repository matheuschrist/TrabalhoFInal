...
$('#registarDespesa').modal('show')
   ...
   document.getElementById('modalCor').className="modal-header text-danger"
    document.getElementById('modalConteudo').innerHTML='Há algum campo obrigatório invalido!'
...
function carregaListaDispesas(reservas=Array(), filtro=false){
	...
	reservas.forEach(function(d){
		...
		switch(d.equipamento){
			case '1': d.equipamento='Notbook'
			break
			case '2': d.equipamento='Tablet'
			break
			case '3': d.equipamento='headset'
			break
			case '4': d.equipamento='Microfone'
			break
